class NetworkVisualizer {
    ...
    // Convert pixel distance to real-world distance (100m to 1km)
    pixelToRealDistance(pixelDistance) {
        // Define canvas diagonal as maximum pixel distance
        const maxPixelDistance = Math.sqrt(this.canvas.width * this.canvas.width + this.canvas.height * this.canvas.height);
        
        // Calculate base distance (linear mapping from pixels to 100m-1km range)
        const normalizedDistance = pixelDistance / maxPixelDistance;
        const baseDistance = 100 + normalizedDistance * 900;
        
        // Add small variation (±10%) to avoid uniform distances
        const variation = 0.2 * (Math.abs(Math.sin(pixelDistance * 0.1)) - 0.5);
        const finalDistance = baseDistance * (1 + variation);
        
        // Ensure distance stays within 100m-1km range
        return Math.round(Math.max(100, Math.min(1000, finalDistance)));
    }
    ...
    drawConnections(nodes) {
        for (const node of nodes) {
            for (const connectedNode of node.connectedNodes) {
                ...
                const realDistance = this.pixelToRealDistance(pixelDistance);
                ...
                // Draw signal strength and distance indicators
                ...
                this.ctx.fillText(`${realDistance}m`, midX, midY + 8);
            }
        }
    }
